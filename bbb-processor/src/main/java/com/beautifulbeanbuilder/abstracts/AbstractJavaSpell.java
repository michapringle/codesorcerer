...
public abstract class AbstractJavaSpell<T extends Annotation, Input> extends AbstractSpell<T, Input, TypeSpec.Builder> {
    ...
    @Override
    public void modify(CodeSorcererProcessor.Result<AbstractSpell<T, Input, TypeSpec.Builder>, Input, TypeSpec.Builder> result, Collection<CodeSorcererProcessor.Result> results) throws Exception {
        //Do nothing?
    }

    @Override
    public void write(CodeSorcererProcessor.Result<AbstractSpell<T, Input, TypeSpec.Builder>, Input, TypeSpec.Builder> result) throws Exception {
        //Do nothing?
    }

    protected static <T extends Annotation, Input, Output, G extends AbstractSpell<T, Input, Output>>
    CodeSorcererProcessor.Result<G, Input, Output>
    getResult(Class<G> generatorClass, TypeElement te, Collection<CodeSorcererProcessor.Result> results) {

        for (CodeSorcererProcessor.Result r : results) {
