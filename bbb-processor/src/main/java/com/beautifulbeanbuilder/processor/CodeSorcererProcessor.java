...
import com.beautifulbeanbuilder.LambdaExceptionUtils;
...
public class CodeSorcererProcessor extends javax.annotation.processing.AbstractProcessor {
    ...
    public static class Result<G extends AbstractSpell, Input, Output> {
    ...
    private void runFunction(Collection<TypeElement> types, LambdaExceptionUtils.Consumer_WithExceptions<Result> f, String phase) {
        ...
        for (TypeElement te : types) {
            for (String ann : getAllAnnotations(te)) {
...
//                System.out.println("--> Casting " + phase + " " + result.spell.getClass().getSimpleName() + " on " + result.te);
